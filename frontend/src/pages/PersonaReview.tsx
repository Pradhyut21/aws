...
const PERSONA_AVATARS: Record<string, string> = {
    ...
    P06: '👩‍💼', P07: '🧓', P08: '🛵', P09: '👗', P10: '🧺',
    P11: '🏥', P12: '🍃', P13: '🚀', P14: '🎨', P15: '👩‍🏫',
    P16: '🛺', P17: '🌈', P18: '🙏', P19: '🕌', P20: '🏔️',
};

function verdictColor(v: string) {
    if (v === 'PASS')      return { bg: 'rgba(74,222,128,0.12)',  border: 'rgba(74,222,128,0.3)',  text: '#4ade80', dot: '#4ade80' };
    if (v === 'FLAG')      return { bg: 'rgba(251,191,36,0.12)',  border: 'rgba(251,191,36,0.3)',  text: '#fbbf24', dot: '#fbbf24' };
    return                        { bg: 'rgba(248,113,113,0.12)', border: 'rgba(248,113,113,0.3)', text: '#f87171', dot: '#f87171' };
}
    ...
    return (
        <svg width={size} height={size} style={{ flexShrink: 0 }}>
            ...
            <text x="50%" y="50%" textAnchor="middle" dominantBaseline="central"
                fill={color} fontSize={size < 50 ? 11 : 14} fontWeight={700} fontFamily="Inter, system-ui">
                {score}
            </text>
        </svg>
    );
...
function PersonaCard({ p, delay = 0 }: { p: PersonaResult; delay?: number }) {
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginTop: '0.2rem' }}>
                        ...
                        <span style={{ color: vc.text, fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.05em' }}>
                            {p.verdict.replace('_', ' ')}
                        </span>
                    </div>
                </div>
            ...
            {/* Feedback */}
            <div style={{ color: '#94a3b8', fontSize: '0.73rem', lineHeight: 1.5, padding: '0.4rem 0' }}>
                "{p.feedback}"
            </div>
            ...
            {p.suggestedTweak && (
                <div style={{
                    ...
                    <span style={{ color: '#818cf8', fontSize: '0.67rem', fontWeight: 600 }}>💡 </span>
                    <span style={{ color: '#64748b', fontSize: '0.67rem' }}>{p.suggestedTweak}</span>
                </div>
            )}
        </div>
    );
...
export default function PersonaReview() {
    ...
    const startReview = async () => {
        ...
        try {
            ...
            const existing = await api.get(`/campaign/${campaignId}/persona-review`).catch(() => null);
            if (existing?.status === 200 && existing.data.overallResonance !== undefined) {
                setData(existing.data); setLoading(false); return;
            }
            // Trigger swarm
    ...
    useEffect(() => { if (campaignId && !data) startReview(); }, [campaignId]);

    const filtered = data?.personaResults.filter(p =>
        filter === 'ALL' ? true : p.verdict === filter
    ) ?? [];

    // ─── RENDER ────────────────────────────────────────────────────────────────
    return (
        <div style={{ minHeight: '100vh', background: '#0a0a14', fontFamily: 'Inter, system-ui, sans-serif', padding: '2rem 1.5rem' }}>
            <style>{`
            ...
            <button onClick={() => navigate(-1)} style={{
                ...
                fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '1.5rem', padding: 0,
            }}>
                ← Back to Campaign
            </button>
            ...
            <div style={{ marginBottom: '2rem' }}>
                ...
                <p style={{ color: '#64748b', fontSize: '0.85rem', margin: 0 }}>
                    ...
                    &nbsp;·&nbsp; Inspired by <span style={{ color: '#818cf8' }}>aws-samples/sample-agentic-genai-agentcore</span>
                </p>
            </div>
            ...
            {(loading || polling) && !data && (
                <div style={{ textAlign: 'center', padding: '4rem 2rem' }}>
                    <div style={{
                        width: 56, height: 56, border: '3px solid rgba(99,102,241,0.2)',
                        borderTopColor: '#818cf8', borderRadius: '50%', margin: '0 auto 1.5rem',
                        animation: 'spin 0.8s linear infinite',
                    }} />
                    <div style={{ color: '#e2e8f0', fontSize: '1rem', fontWeight: 600, marginBottom: '0.5rem' }}>
                        {loading ? 'Starting BharatPersonaSwarm…' : 'Running 20 Indian personas in parallel…'}
                    </div>
                    <div style={{ color: '#475569', fontSize: '0.8rem' }}>
                        20 Nova Lite calls running simultaneously · Validator + Finalizer agents queued
                    </div>
                    {/* Skeleton persona cards */}
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '0.875rem', marginTop: '2rem', textAlign: 'left' }}>
                        {Array.from({ length: 6 }).map((_, i) => (
                            <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)',
                                borderRadius: '0.875rem', padding: '1rem', animation: 'shimmer 1.4s ease infinite',
                                animationDelay: `${i * 0.1}s` }}>
                                <div style={{ height: 48, background: 'rgba(255,255,255,0.05)', borderRadius: '0.5rem' }} />
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {error && (
                <div style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.2)',
                    borderRadius: '0.75rem', padding: '1rem 1.25rem', color: '#f87171', marginBottom: '1.5rem' }}>
                    {error}
                </div>
            )}
            ...
            {data && (
                <>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '1.5rem',
                        ...
                        <div>
                            <div style={{ color: '#94a3b8', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.08em', marginBottom: '0.25rem' }}>
                                BHARATRESONANCE SCORE™
                            </div>
                            <div style={{ color: scoreColor(data.overallResonance), fontSize: '2rem', fontWeight: 800, lineHeight: 1.1 }}>
                            ...
                            <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
                                {[
                                    ...
                                    <span key={item.label} style={{ color: item.color, fontSize: '0.78rem', fontWeight: 700 }}>
                                        {item.label}
                                    </span>
                                ))}
                                <span style={{ color: '#475569', fontSize: '0.75rem' }}>
                                    · {(data.latencyMs / 1000).toFixed(1)}s · {data.personaResults.length} personas
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.15)',
                        ...
                        <div style={{ color: '#10b981', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.06em', marginBottom: '0.5rem' }}>
                    ...
                    {data.validatorFlags.length > 0 && (
                        <div style={{ background: 'rgba(248,113,113,0.06)', border: '1px solid rgba(248,113,113,0.15)',
                            borderRadius: '1rem', padding: '1.25rem', marginBottom: '1.5rem' }}>
                            <div style={{ color: '#f87171', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.06em', marginBottom: '0.6rem' }}>
                                ⚠ CULTURAL / LEGAL FLAGS ({data.validatorFlags.length})
                            </div>
                            {data.validatorFlags.map((f, i) => (
                                <div key={i} style={{ color: '#fca5a5', fontSize: '0.8rem', lineHeight: 1.5,
                                    padding: '0.3rem 0', borderBottom: i < data.validatorFlags.length - 1 ? '1px solid rgba(248,113,113,0.1)' : 'none' }}>
                                    • {f}
                                </div>
                            ))}
                        </div>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)',
                        borderRadius: '1rem', padding: '1.25rem', marginBottom: '1.5rem' }}>
                        <div style={{ color: '#64748b', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.06em', marginBottom: '0.75rem' }}>
                            ALL PERSONAS — RESONANCE SCORES
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                            {[...data.personaResults].sort((a, b) => b.resonanceScore - a.resonanceScore).map(p => (
                                <div key={p.personaId} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                                    <span style={{ fontSize: '0.9rem', width: 22, textAlign: 'center', flexShrink: 0 }}>
                                        {PERSONA_AVATARS[p.personaId] ?? '👤'}
                                    </span>
                                    <div style={{ fontSize: '0.7rem', color: '#64748b', width: 130, flexShrink: 0,
                                        whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                        {p.name}
                                    </div>
                                    <div style={{ flex: 1, height: 6, background: 'rgba(255,255,255,0.05)', borderRadius: 999 }}>
                                        <div style={{
                                            height: '100%', width: `${p.resonanceScore}%`,
                                            background: `linear-gradient(90deg, ${scoreColor(p.resonanceScore)}88, ${scoreColor(p.resonanceScore)})`,
                                            borderRadius: 999, transition: 'width 0.6s ease',
                                        }} />
                                    </div>
                                    <span style={{ color: scoreColor(p.resonanceScore), fontSize: '0.7rem', fontWeight: 700, width: 28, textAlign: 'right', flexShrink: 0 }}>
                                        {p.resonanceScore}
                                    </span>
                                </div>
                            ))}
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                        {(['ALL', 'PASS', 'FLAG', 'HIGH_RISK'] as const).map(v => {
                            const count = v === 'ALL' ? data.personaResults.length
                    ...
                    {/* Footer meta */}
                    <div style={{ textAlign: 'center', marginTop: '2rem', color: '#334155', fontSize: '0.72rem' }}>
                        Generated {new Date(data.createdAt).toLocaleString('en-IN')} ·
                        Powered by Amazon Nova Lite (persona calls) + Nova Pro (validator + finalizer) ·
