...
function generateDebate(copy: string): ChatMessage[] {
    ...
    if (hasPrice) {
        messages.push({
            persona: 'ramesh',
            text: `Bhai yeh offer thoda acha lag raha hai! Lekin discount ka proof chahiye — logo abhi card nahi maante, cash pe trust hai 💪`,
            vote: 'tweak',
        });
    } else if (isEnglishOnly) {
        messages.push({
            persona: 'ramesh',
            text: `Yaar yeh English mein kya likha hai? Hamare Kanpur mein customers Hindi mein hi baat karte hain. Hindi mein likho toh 3 guna response milega 🙏`,
            vote: 'reject',
        });
    } else if (isHindi) {
        messages.push({
            persona: 'ramesh',
            text: `Wah bhai! Bhasha bilkul sahi hai. Bas ek kaam karo — dukaan ka WhatsApp number aur delivery time saaf likho. Customers pehle yahi poochhte hain!`,
            vote: 'buy',
        });
    } else {
        messages.push({
            persona: 'ramesh',
            text: `Dekhte hain... thoda aur catchy hona chahiye tha. Price clearly mention karo toh achha lagega`,
            vote: 'tweak',
        });
    }
    ...
    if (hasFestival) {
        messages.push({
            persona: 'ananya',
            text: `YASSSS finally a brand that gets festival vibes!! 🎉 The energy is immaculate but you NEED a trending audio recommendation for the Reel version. Also add a countdown timer aesthetic 🔥`,
            vote: 'buy',
        });
    } else if (isEnglishOnly) {
        messages.push({
            persona: 'ananya',
            text: `Okay the English is giving very corporate energy 😬 For Instagram? Nobody reads full paragraphs. Just give me: one punchy line, one hook emoji, and a CTA. The rest is noise lol`,
            vote: 'reject',
        });
    } else if (hasUrgency) {
        messages.push({
            persona: 'ananya',
            text: `The urgency is REAL and I am here for it ⚡ But the copy needs a meme format — like "POV: you got the Kadak deal before your friends did" — that would literally blow up on Stories`,
            vote: 'buy',
        });
    } else {
        messages.push({
            persona: 'ananya',
            text: `Hmm it's okay? Not bad but nothing that makes me want to screenshot and share tbh. Needs a stronger hook in the first 3 words. That's where 80% of people stop reading 📱`,
            vote: 'tweak',
        });
    }
    ...
    if (hasFestival) {
        messages.push({
            persona: 'murugan',
            text: `Nalla irukku! Festival angle super idea. Enna, delivery date clearly specify panningala? Deepavali ku munnaadi reach aguma? Coimbatore la people order cancel panniduvanga if it's not clear 🙅`,
            vote: 'tweak',
        });
    } else if (hasPrice && !hasUrgency) {
        messages.push({
            persona: 'murugan',
            text: `Discount percentage okay da, but for wholesale customers — minimum order quantity mention pannu. And cash-on-delivery available-a? Rural Tamil Nadu la that's non-negotiable bro 💯`,
            vote: 'tweak',
        });
    } else if (isTamil) {
        messages.push({
            persona: 'murugan',
            text: `Ayyo super! Tamil la irukku means already half the trust is earned 🎊 Just add return policy clarity — Coimbatore market la trust = clear policy. Add that and this campaign is 🔥`,
            vote: 'buy',
        });
    } else {
        messages.push({
            persona: 'murugan',
            text: `From a Tier-2 market perspective — this is decent but too metro-focused. Rural customers care about: (1) delivery time, (2) COD, (3) local language. Miss even one → skip. Fix these and it's a solid campaign`,
            vote: 'tweak',
        });
    }
    ...
    if (buyCount >= 2) {
        messages.push({
            persona: 'ananya',
            text: `Okay group consensus: this is actually solid! Ramesh bhai and Murugan anna both gave thumbs up — that's basically the whole Bharat market right there 😂🙌`,
            vote: null,
        });
    } else if (rejectCount >= 2) {
        messages.push({
            persona: 'ramesh',
            text: `Bhai log sab agree kar rahe hain — thoda aur kaam karna padega. Bhasha aur price dono improve karo. Phir dobara try karo!`,
            vote: null,
        });
    } else {
        messages.push({
            persona: 'murugan',
            text: `Okay team — 50-50 verdict. This campaign needs 2 tweaks: regional language & delivery clarity. Fix those and we vote again 🔄`,
            vote: null,
        });
    }
...
export default function BharatFocusGroup() {
    ...
    const voteTally = messages.reduce(
        (acc, m) => {
            if (m.vote) acc[m.vote] = (acc[m.vote] || 0) + 1;
            return acc;
        },
    ...
    const runFocusGroup = useCallback(async () => {
        ...
        toast.promise(
            new Promise<string>(resolve =>
                setTimeout(() => resolve('Focus group complete!'), allMessages.length * 1400 + 500)
            ),
            {
                loading: '🗣️ Personas are debating your copy…',
                success: '✅ Focus group concluded!',
                error: 'Error running focus group',
            }
        );
    ...
    return (
        <div
            style={{
                minHeight: '100vh',
                display: 'flex',
                background: 'linear-gradient(135deg,#0a0a14 0%,#0d0a1a 50%,#0a1410 100%)',
            }}
        >
            <Sidebar />
            <main
            ...
            >
                ...
                >
                    ...
                    <p style={{ color: '#64748b', margin: '0.25rem 0 0', fontSize: '0.9rem' }}>
                        Paste your campaign copy below — 3 real Indian consumer personas will debate
                        it live in WhatsApp style.
                    </p>
                </motion.div>
                ...
                >
                    ...
                    <div>
                        ...
                        <div
                            style={{
                                background:
                                    'linear-gradient(135deg, rgba(13,31,22,0.95), rgba(10,20,15,0.98))',
                        ...
                        >
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    >
                                        Bharat Market Focus Group
                                    </div>
                                    <div style={{ color: '#25D366', fontSize: '0.72rem' }}>
                                        {PERSONAS.length} members · AI-simulated consumer debate
                                    </div>
                                </div>
                                <div
                                    style={{
                            ...
                            >
                                {messages.length === 0 && !running && (
                                    ...
                                    >
                                        ...
                                        <div style={{ color: '#2d5040', fontSize: '0.85rem' }}>
                                            Paste your campaign copy above and hit Run
                                        </div>
                                    </div>
                                )}
                                ...
                                <AnimatePresence>
                                    {typing && (
                                        ...
                                        >
                                            ...
                                            <div
                                                style={{
                                                    background: PERSONAS.find(p => p.id === typing)
                                                        ?.bgColor,
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <div style={{ color: p.color, fontSize: '0.72rem' }}>
                                            {p.role}
                                        </div>
                                            ...
                                            color:
                                                consensusScore >= 70
                                                    ...
                                                    : consensusScore >= 50
                                                      ...
                                                      : '#f87171',
                                            lineHeight: 1,
                                            textShadow: `0 0 24px ${consensusScore >= 70 ? '#4ade8080' : '#fbbf2480'}`,
                                        }}
                                    >
                                        {consensusScore}
                                    </div>
