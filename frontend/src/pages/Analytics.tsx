...
export default function Analytics() {
    ...
    // Weekly engagement trend (simulated based on campaign count)
    const weeklyTrend = useMemo(() => {
        const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
        return days.map((day, i) => ({
            day,
            reach: total > 0 ? Math.floor((total * 8 + i * total * 3 + Math.sin(i) * total * 5)) : 0,
            engagement: total > 0 ? Math.floor(total * 2 + i * total + Math.cos(i) * total * 2) : 0,
        }));
    }, [total]);

    // Platform radar data
    const platformRadar = [
        { platform: 'Instagram', score: campaigns.filter((c: any) => c.result?.content?.captions?.instagram).length * 20 + 40 },
        { platform: 'WhatsApp', score: campaigns.filter((c: any) => c.result?.content?.captions?.whatsapp).length * 20 + 35 },
        { platform: 'Facebook', score: campaigns.filter((c: any) => c.result?.content?.captions?.facebook).length * 20 + 30 },
        { platform: 'YouTube', score: campaigns.filter((c: any) => c.result?.content?.captions?.youtube).length * 15 + 25 },
        { platform: 'LinkedIn', score: campaigns.filter((c: any) => c.result?.content?.captions?.linkedin).length * 15 + 20 },
        ...
        <div className="min-h-screen flex">
            ...
            <main className="flex-1 ml-[220px] p-8">
                ...
                {isEmpty ? (
                    ...
                    <>
                        {/* Stats — CountUp animated on mount */}
                        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                            {[
                                { label: 'Campaigns', value: total, icon: '🚀', color: '#FF6B35', delta: 'created' },
                                { label: 'States', value: Object.keys(stateCounts).length, icon: '🗺️', color: '#00D4FF', delta: 'targeted' },
                                { label: 'Languages', value: Object.keys(langCounts).length, icon: '🌐', color: '#F7C948', delta: 'used' },
                                { label: 'Biz Types', value: Object.keys(bizCounts).length, icon: '🏪', color: '#00FF88', delta: 'categories' },
                                { label: 'Avg Score', value: avgScore, icon: '⭐', color: '#A855F7', delta: '/100' },
                            ].map((stat, i) => (
                                <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                                    transition={{ delay: i * 0.08 }} className="glass-card p-5 feature-card relative overflow-hidden">
                                    ...
                                    <div className="text-3xl font-black font-poppins" style={{ color: stat.color }}>
                                        {statsVisible && stat.value > 0 ? (
                                            <CountUp start={0} end={stat.value} duration={1.4} delay={i * 0.08} />
                                        ) : (
                        ...
                        {/* NEW: Weekly Engagement AreaChart + Platform Radar */}
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                            {/* AreaChart — weekly engagement trend */}
                            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.65 }} className="glass-card p-5">
                                <h3 className="font-bold font-poppins text-white mb-1">Weekly Engagement Trend</h3>
                                <p className="text-slate-500 text-xs mb-4">Est. reach & engagement by day based on your campaigns</p>
                                <ResponsiveContainer width="100%" height={200}>
                                    <AreaChart data={weeklyTrend} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                                        ...
                                        <Area type="monotone" dataKey="reach" stroke="#FF6B35" strokeWidth={2} fill="url(#reachGrad)" dot={false} activeDot={{ r: 4 }} />
                                        <Area type="monotone" dataKey="engagement" stroke="#A855F7" strokeWidth={2} fill="url(#engageGrad)" dot={false} activeDot={{ r: 4 }} />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </motion.div>

                            {/* RadarChart — platform coverage */}
                            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.72 }} className="glass-card p-5">
                                <h3 className="font-bold font-poppins text-white mb-1">Platform Coverage</h3>
                                <p className="text-slate-500 text-xs mb-4">How well you're covering each social channel</p>
                                <ResponsiveContainer width="100%" height={200}>
                                    <RadarChart data={platformRadar} cx="50%" cy="50%" outerRadius="70%">
