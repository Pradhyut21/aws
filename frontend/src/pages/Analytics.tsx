...
const STATE_POSITIONS: Record<string, [number, number]> = {
    ...
    'Tamil Nadu': [45, 75],
    'West Bengal': [65, 40],
    'Andhra Pradesh': [50, 65],
    'Madhya Pradesh': [50, 42],
    Kerala: [42, 73],
    Telangana: [48, 62],
    Odisha: [60, 52],
    Bihar: [62, 30],
...
export default function Analytics() {
    ...
    const avgScore = useMemo(() => {
        ...
        const sum = scored.reduce(
            (acc: number, c: any) => acc + (c.result.bharatScore.total ?? 0),
            0
        );
    ...
    const platformRadar = [
        {
            platform: 'Instagram',
            score:
                campaigns.filter((c: any) => c.result?.content?.captions?.instagram).length * 20 +
                40,
        },
        {
            platform: 'WhatsApp',
            score:
                campaigns.filter((c: any) => c.result?.content?.captions?.whatsapp).length * 20 +
                35,
        },
        {
            platform: 'Facebook',
            score:
                campaigns.filter((c: any) => c.result?.content?.captions?.facebook).length * 20 +
                30,
        },
        {
            platform: 'YouTube',
            score:
                campaigns.filter((c: any) => c.result?.content?.captions?.youtube).length * 15 + 25,
        },
        {
            platform: 'LinkedIn',
            score:
                campaigns.filter((c: any) => c.result?.content?.captions?.linkedin).length * 15 +
                20,
        },
            ...
            <main className="flex-1 ml-[220px] p-8">
                ...
                >
                    ...
                    <p className="text-slate-400 mt-1">
                        {isEmpty
                            ? 'Generate your first campaign to see analytics here!'
                            : `Based on ${total} campaign${total !== 1 ? 's' : ''} you've created`}
                    </p>
                </motion.div>
                ...
                {isEmpty ? (
                    ...
                    >
                        ...
                        <p className="text-slate-400 mb-6">
                            Create your first campaign and your analytics will appear here
                            automatically.
                        </p>
                        <a href="/campaign/new" className="btn-primary">
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                            {[
                                ...
                                {
                                    label: 'States',
                                    value: Object.keys(stateCounts).length,
                                    icon: '🗺️',
                                    color: '#00D4FF',
                                    delta: 'targeted',
                                },
                                ...
                                {
                                    label: 'Biz Types',
                                    value: Object.keys(bizCounts).length,
                                    icon: '🏪',
                                    color: '#00FF88',
                                    delta: 'categories',
                                },
                            ...
                            >
                                ...
                                {stateBar.length > 0 ? (
                                    ...
                                    <p className="text-slate-500 text-sm py-16 text-center">
                                        Select states when creating campaigns
                                    </p>
                                )}
                            </motion.div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="glass-card p-5 relative">
                                <h3 className="font-bold font-poppins text-white mb-4">
                                    Your Target States — India Map
                                </h3>
                                <div className="relative h-64 bg-[#060c1a] rounded-xl overflow-hidden">
                                    <svg viewBox="0 0 100 100" className="w-full h-full opacity-30">
                                        <path
                                            d="M35 15 C45 12 60 14 70 18 C80 22 85 30 87 40 C89 50 85 60 78 68 C70 76 60 82 50 86 C40 82 30 76 22 68 C15 60 11 50 13 40 C15 30 20 22 30 18 Z"
                                            fill="#0D1B40"
                                            stroke="#FF6B35"
                                            strokeWidth="0.5"
                                        />
                                        ...
                                        return (
                                            ...
                                            >
                                                <div
                                                    ...
                                                    title={
                                                        isUsed
                                                            ? `${state}: ${count} campaign${count > 1 ? 's' : ''}`
                                                            : state
                                                    }
                                                    style={{
                                                        width: size,
                                                        ...
                                                        boxShadow: isUsed
                                                            ? `0 0 ${size * 2}px rgba(255,107,53,0.7)`
                                                            : 'none',
                                                    }}
                                                />
                                                {isUsed && (
                                                    <span className="text-[7px] text-orange-300 mt-0.5 whitespace-nowrap font-mono">
                                                        {state}
                                                    </span>
                                                )}
                                            </div>
                                ...
                                <p className="text-xs text-slate-500 mt-2 font-mono">
                                    🔴 States you've targeted · Brighter = more campaigns
                                </p>
                            </div>
                            ...
                            >
                                ...
                                {bizBar.length > 0 ? (
                                    ...
                                    <p className="text-slate-500 text-sm py-16 text-center">
                                        Business type data will appear here
                                    </p>
                                )}
                            </motion.div>
                        </div>
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-white text-sm font-medium truncate">
                                                {c.input?.slice(0, 60) || 'Campaign'}
                                            </p>
                                            <p className="text-slate-500 text-xs">
                                                {c.businessType} · {(c.regions || []).join(', ')} ·{' '}
                                                {LANG_NAMES[c.language || 'en'] || c.language}
                                            </p>
                                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                            ...
                            >
                                ...
                                <p className="text-slate-500 text-xs mb-4">
                                    Est. reach & engagement by day based on your campaigns
                                </p>
                                <ResponsiveContainer width="100%" height={200}>
                                    <AreaChart
                                        data={weeklyTrend}
                            ...
                            >
                                ...
                                <p className="text-slate-500 text-xs mb-4">
                                    How well you're covering each social channel
                                </p>
                                <ResponsiveContainer width="100%" height={200}>
                                    <RadarChart
                                        data={platformRadar}
