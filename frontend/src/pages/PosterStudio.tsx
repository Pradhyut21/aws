    ...
    <div style={{
      ...
      {['0 0', `${w - 20}px 0`, `0 ${h - 20}px`, `${w - 20}px ${h - 20}px`].map((pos, i) => (
        <div key={i} style={{
          ...
          color: theme.accentColor, fontSize: '0.9rem', opacity: 0.7, userSelect: 'none',
        }}>✦</div>
      ))}
      ...
      <div style={{ textAlign: 'center', zIndex: 1 }}>
        ...
        <div style={{
          marginTop: '0.5rem', color: theme.accentColor,
          fontSize: is916 ? '0.65rem' : '0.6rem', fontWeight: 700,
          textTransform: 'uppercase', letterSpacing: '0.15em', opacity: 0.7,
        }}>{theme.lang} · {theme.dateHint}</div>
      </div>

      {/* Festival name + headline */}
      <div style={{ textAlign: 'center', zIndex: 1 }}>
        <div style={{
          color: theme.accentColor, fontSize: is916 ? '0.72rem' : '0.65rem',
          fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em',
          marginBottom: '0.5rem', opacity: 0.8,
        }}>{theme.name}</div>
        <div style={{
          color: '#fff', fontSize: is916 ? '1.3rem' : '1.1rem', fontWeight: 900,
          lineHeight: 1.25, marginBottom: '0.6rem', textShadow: `0 0 20px ${theme.accentColor}`,
        }}>
          {theme.headline}
        </div>
        <div style={{
          color: theme.accentColor + 'cc', fontSize: is916 ? '0.65rem' : '0.6rem',
      ...
      <div style={{ textAlign: 'center', zIndex: 1, width: '100%' }}>
        <div style={{ color: '#fff', fontSize: is916 ? '0.95rem' : '0.85rem', fontWeight: 800 }}>
          {brandName || 'Your Brand Name'}
        </div>
        {tagline && (
          <div style={{ color: theme.accentColor + 'aa', fontSize: is916 ? '0.6rem' : '0.55rem', marginTop: 2 }}>
            {tagline}
          </div>
        )}
        {phone && (
          <div style={{
            marginTop: '0.35rem', display: 'inline-flex', alignItems: 'center', gap: '0.3rem',
            background: `${theme.accentColor}20`, borderRadius: 999,
            padding: '0.15rem 0.65rem', border: `1px solid ${theme.accentColor}40`,
          }}>
            <span style={{ fontSize: '0.7rem' }}>📱</span>
            <span style={{ color: theme.accentColor, fontSize: is916 ? '0.65rem' : '0.6rem', fontWeight: 700 }}>
              {phone}
            </span>
          </div>
        )}
...
export default function PosterStudio() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: 'linear-gradient(135deg,#0a0814 0%,#0a1408 50%,#140a08 100%)' }}>
      <Sidebar />
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.3rem' }}>
            ...
            <div style={{
              ...
              borderRadius: 999, padding: '0.2rem 0.7rem', fontSize: '0.7rem', color: '#f59e0b', fontWeight: 700,
            }}>🎨 POSTER STUDIO</div>
          </div>
          ...
          <p style={{ color: '#64748b', margin: '0.25rem 0 0', fontSize: '0.9rem' }}>
            1-click WhatsApp Status & Instagram Feed poster generator for Indian festivals. Download as 9:16 or 1:1.
          </p>
        </motion.div>

        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '1.5rem', maxWidth: 1100 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                {FESTIVALS.map((f, i) => (
                  <button key={f.id} onClick={() => setFestival(i)} style={{
                    background: festival === i ? f.accentColor + '18' : 'rgba(255,255,255,0.03)',
                    border: `1px solid ${festival === i ? f.accentColor : 'rgba(255,255,255,0.07)'}`,
                    borderRadius: '0.65rem', padding: '0.55rem 0.85rem', cursor: 'pointer', textAlign: 'left',
                    display: 'flex', alignItems: 'center', gap: '0.65rem', transition: 'all 0.15s',
                    boxShadow: festival === i ? `0 0 12px ${f.accentColor}25` : 'none',
                  }}>
                    <span style={{ fontSize: '1.2rem' }}>{f.emoji}</span>
                    <div>
                      <div style={{ color: festival === i ? f.accentColor : '#e2e8f0', fontWeight: 700, fontSize: '0.82rem' }}>{f.name}</div>
                      <div style={{ color: '#64748b', fontSize: '0.68rem' }}>{f.dateHint}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div style={{
              ...
              {[
                { label: 'Brand / Shop Name', value: brandName, set: setBrandName, placeholder: 'e.g. Raju Silk House' },
                { label: 'Tagline (optional)', value: tagline, set: setTagline, placeholder: 'e.g. Pure & Authentic Since 1985' },
                { label: 'WhatsApp Number', value: phone, set: setPhone, placeholder: 'e.g. +91 98765 43210' },
              ].map(field => (
                <div key={field.label} style={{ marginBottom: '0.6rem' }}>
                  <label style={{ color: '#94a3b8', fontSize: '0.72rem', fontWeight: 600, display: 'block', marginBottom: '0.25rem' }}>{field.label}</label>
                  <input
                    value={field.value} onChange={e => field.set(e.target.value)}
                    placeholder={field.placeholder}
                    style={{
                      width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)',
                      borderRadius: '0.5rem', padding: '0.55rem 0.75rem', color: '#f1f5f9', fontSize: '0.85rem',
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', alignItems: 'start' }}>
              ...
              <div>
                ...
                <div ref={poster916Ref} style={{ display: 'inline-block' }}>
                  <PosterCanvas theme={theme} brandName={brandName} tagline={tagline} phone={phone} mode="9:16" />
                </div>
                <motion.button
                  ...
                  style={{
                    marginTop: '0.75rem', width: 240,
                    background: downloading === '9:16' ? 'rgba(255,255,255,0.06)' : 'rgba(37,211,102,0.12)',
                ...
                >
                  {downloading === '9:16' ? '⏳ Generating…' : '📤 Download WhatsApp'}
                </motion.button>
              </div>
              ...
              <div>
                ...
                <div ref={poster11Ref} style={{ display: 'inline-block' }}>
                  <PosterCanvas theme={theme} brandName={brandName} tagline={tagline} phone={phone} mode="1:1" />
                </div>
                <motion.button
                  whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                  ...
                  style={{
                    marginTop: '0.75rem', width: 280,
                    background: downloading === '1:1' ? 'rgba(255,255,255,0.06)' : 'rgba(225,48,108,0.12)',
                ...
                >
                  {downloading === '1:1' ? '⏳ Generating…' : '📸 Download Instagram'}
                </motion.button>
              </div>
            </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
              style={{
                ...
                color: '#64748b', fontSize: '0.78rem', lineHeight: 1.6,
              }}>
              💡 <strong style={{ color: '#94a3b8' }}>Powered by html-to-image</strong> — downloads high-res PNG at 3× pixel density. No server needed, runs entirely in your browser. Add your brand name above to personalize!
            </motion.div>
          </div>
        </div>
      </main>
