...
import { createCampaign, translateCaptions, analyzeCopy } from '../lib/api';
...
export default function NewCampaign() {
    ...
    // ─── AWS Translate + Comprehend state ──────────────────────────────────────
    const [translations, setTranslations] = useState<Record<string, string> | null>(null);
    const [translating, setTranslating] = useState(false);
    const [comprehendResult, setComprehendResult] = useState<{
    ...
    useKeyboardShortcuts([
        {
            ...
            handler: () => {
                if (step === 1 && textInput) handleSubmit();
            },
        },
    ]);
    ...
    useEffect(() => {
        ...
        const matchedBiz = BUSINESS_TYPES.find(
            b =>
                lowerInput.includes(b.id) ||
                b.label
                    .toLowerCase()
                    .split(/[/\s]+/)
                    .some(word => word.length > 3 && lowerInput.includes(word))
        );
        if (matchedBiz) {
            ...
            lowerInput.includes('bulding') ||
            lowerInput.includes('matirals')
        ) {
    ...
    const handleSubmit = useCallback(async () => {
        setStep(2);
        simulatePipeline();
        // Sonner promise toast: tracks the AI pipeline
        const pipelineToastId = toast.loading(
            '🤖 AI Pipeline running… Research → Creative → Quality → Distribution',
            {
                description: 'Your campaign is being generated by 4 AI agents',
            }
        );
        ...
        const cleanWords = textInput
            .toLowerCase()
            .replace(/[^\w\s]/gi, '')
            .replace(
                /\b(i|want|need|give|me|show|an|a|the|some|any|make|create|generate|ad|campaign|for|my|in|at|this|that|please|pls|do|it|and|to|with|about|details|wikipedia|importance)\b/gi,
                ''
            )
            .split(/\s+/)
            .filter(w => w.length > 2)
        ...
        const personalizedMock = {
            ...
            content: {
                ...
                images: [
                    {
                        id: 'img-1',
                        url: `https://loremflickr.com/800/600/${encodeURIComponent(biz)},${encodeURIComponent(region0)}?lock=1`,
                        platform: 'instagram',
                        caption: igCaption,
                    },
                    {
                        id: 'img-2',
                        url: `https://loremflickr.com/800/600/${encodeURIComponent(biz)},india?lock=2`,
                        platform: 'facebook',
                        caption: fbCaption,
                    },
                    {
                        id: 'img-3',
                        url: `https://loremflickr.com/800/600/${encodeURIComponent(biz)},${encodeURIComponent(region0)}?lock=3`,
                        platform: 'whatsapp',
                        caption: waMessage,
                    },
                    {
                        id: 'img-4',
                        url: `https://loremflickr.com/800/600/${encodeURIComponent(biz)},business?lock=4`,
                        platform: 'youtube',
                        caption: `${biz} — Best in ${regionStr} 🌟`,
                    },
                ],
                captions: {
                    instagram: igCaption,
        ...
        const score = finalData?.bharatScore?.total ?? '--';
        toast.dismiss(pipelineToastId);
        toast.success('🎉 Campaign ready!', {
            description: `BharatScore: ${score}/100 · ${selectedPlatforms.length} platforms · ${regions[0] || 'India'}`,
            duration: 5000,
        });
    ...
    const handlePublishAll = () => {
        ...
        const caption =
            campaignResult?.content?.captions?.instagram ||
    ...
    const handleDownload = () => {
        ...
        a.href = url;
        a.download = 'bharatmedia-campaign.txt';
        a.click();
        URL.revokeObjectURL(url);
        toast.success('Campaign exported! 📥', {
            description: 'Saved as bharatmedia-campaign.txt',
        });
    };
                ...
                <div className="flex items-center gap-4 mb-8">
                    {[1, 2, 3].map(s => (
                        <div key={s} className="flex items-center gap-2">
                            <div
                                className={`w - 8 h - 8 rounded - full flex items - center justify - center text - sm font - bold font - poppins transition - all ${
                                    step >= s
                                        ? 'text-black'
                            ...
                            <span
                                className={`text - sm font - medium ${step >= s ? 'text-white' : 'text-slate-500'} `}
                            >
                                {s === 1 ? 'Your Idea' : s === 2 ? 'Creating...' : 'Your Campaign'}
                            </span>
                            {s < 3 && (
                                <div
                                    className={`w - 12 h - 0.5 ${step > s ? 'bg-orange-500' : 'bg-slate-700'} `}
                                />
                            )}
                        </div>
                    ))}
                ...
                <AnimatePresence mode="wait">
                            ...
                            <div className="max-w-5xl mx-auto">
                                <div className="text-center mb-10">
                                    <motion.div
                                        ...
                                        className="inline-block px-4 py-1.5 rounded-full border border-orange-500/30 bg-orange-500/10 text-orange-400 font-medium text-sm mb-4"
                                    >
                                    ...
                                    <p className="text-slate-400 text-lg max-w-2xl mx-auto">
                                        Describe your business and let our advanced AI orchestrator
                                        design a highly converting, multi-platform campaign tailored
                                        to your audience.
                                    </p>
                                </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                                        ...
                                                        {langDetect.detected && (
                                                            ...
                                                            >
                                                                ...
                                                                <span className="text-xs px-2.5 py-1 rounded-full text-orange-400 border border-orange-500/20 bg-orange-500/10 font-mono font-medium flex items-center gap-1.5">
                                                                    {langDetect.languageName}{' '}
                                                                    <span className="text-white/50">
                                                                        |
                                                                    </span>{' '}
                                                                    {Math.round(
                                                                        langDetect.confidence * 100
                                                                    )}
                                                                    %
                                                                </span>
                                                            </motion.div>
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    {[
                                                        ...
                                                        <button
                                                            ...
                                                            className={`p-2.5 text-left rounded-xl text-sm transition-all border ${
                                                                campaignGoal === g.id
                                    ...
                                    <div className="lg:col-span-4 flex flex-col gap-6">
                                        ...
                                        <div className="relative z-50 glass-card p-5 rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.05] to-transparent">
                                            <label className="text-sm font-bold text-white mb-3 block flex items-center gap-2">
                                                <span className="text-green-400">🏢</span> Business
                                                Sector
                                            </label>
                                            <div className="grid grid-cols-3 gap-2">
                                                {BUSINESS_TYPES.slice(0, 9).map(b => (
                                                    <button
                                                        ...
                                                        className={`p-2 rounded-xl text-center flex flex-col items-center justify-center gap-1 transition-all border ${
                                                            businessType === b.id
                                        ...
                                        <div className="relative z-40 glass-card p-5 rounded-2xl border border-white/10 bg-gradient-to-bl from-white/[0.05] to-transparent">
                                            <label className="text-sm font-bold text-white mb-3 block flex items-center gap-2">
                                                <span className="text-pink-400">🗣️</span> Output
                                                Language
                                            </label>
                                            <LanguageSelector
                                                value={language}
                                        ...
                                        <div className="relative z-30 glass-card p-5 rounded-2xl border border-white/10 bg-gradient-to-tr from-white/[0.05] to-transparent flex-1 flex flex-col min-h-[200px]">
                                            <div className="flex justify-between items-center mb-3">
                                                ...
                                                <span className="text-xs font-semibold text-orange-400 bg-orange-400/10 px-2 py-0.5 rounded-md">
                    ...
                    {step === 2 && (
                        ...
                        >
                            ...
                            {[
                                { e: '🌸', top: '5%', left: '5%', d: 3 },
                                { e: '⚡', top: '10%', right: '8%', d: 2.5 },
                                { e: '🦊', top: '20%', left: '2%', d: 4 },
                                { e: '🔮', top: '15%', right: '3%', d: 3.5 },
                                { e: '🎐', bottom: '20%', left: '4%', d: 2.8 },
                                { e: '🐲', bottom: '15%', right: '5%', d: 3.2 },
                                { e: '🌺', top: '40%', left: '1%', d: 2.2 },
                                { e: '⛩️', top: '35%', right: '2%', d: 3.8 },
                                { e: '💫', bottom: '35%', left: '7%', d: 2 },
                                { e: '🎋', bottom: '30%', right: '6%', d: 4.2 },
                                { e: '🌊', top: '60%', left: '3%', d: 3 },
                                { e: '🎴', top: '65%', right: '4%', d: 2.7 },
                            ].map((s, i) => (
                                <motion.span
                                    key={i}
                            ...
                            <div className="relative flex items-center justify-center mb-8">
                                ...
                                <motion.div
                                    ...
                                    style={{
                                        background:
                                            'radial-gradient(circle, #FF6B35 0%, #A855F7 50%, #00D4FF 100%)',
                                    }}
                                    animate={{
                                        scale: [1, 1.12, 1],
                                        boxShadow: [
                            ...
                            {/* Text */}
                            <motion.h2
                                className="text-4xl font-black font-poppins gradient-text mb-3 text-center"
                            ...
                            >
                                <p className="text-slate-400 text-sm font-mono">
                                    ...
                                    ][Math.min(currentStage, 4)] ||
                                        '🚀 Amazon Bedrock AI is at work...'}
                                </p>
                            </motion.div>
                            ...
                            <div className="w-80 h-2 bg-white/5 rounded-full overflow-hidden mb-6">
                                <motion.div
                                    ...
                                    style={{
                                        background:
                                            'linear-gradient(90deg, #FF6B35, #F7C948, #00D4FF)',
                                    }}
                                    initial={{ width: '0%' }}
                                    animate={{
                                        width: `${Math.min(((currentStage + 1) / 5) * 100, 100)}%`,
                                    }}
                    ...
                    {step === 3 && campaignResult?.content && (
                        ...
                        >
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h2 className="text-3xl font-black font-poppins gradient-text">
                                        Your Campaign is Ready! 🎉
                                    </h2>
                                    <p className="text-slate-400 capitalize">
                                        {campaignResult.title || `${businessType} Campaign`} —{' '}
                                        {campaignGoal} Focus / {tone} Tone
                                    </p>
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    {campaignId && (
                                        <button
                                            onClick={() =>
                                                navigate(`/persona-review/${campaignId}`)
                                            }
                                            style={{
                                                background:
                                            ...
                                            title="Run BharatPersonaSwarm — 20 Indian demographic personas review your campaign"
                                        >
                                            🇮🇳 Persona Review
                                        </button>
                                    )}
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                                ...
                                <div className="lg:col-span-2 glass-card p-6">
                                    <h3 className="font-bold font-poppins text-white mb-4">
                                        Publish To
                                    </h3>
                                    ...
                                    {publishSuccess ? (
                                        ...
                                        >
                                            ...
                                            <h3 className="text-green-400 font-bold text-xl font-poppins">
                                                Campaign Published!
                                            </h3>
                                            <p className="text-slate-400 text-sm mt-2">
                                                Published to {selectedPlatforms.length} platforms.
                                                Estimated reach: 24,000 users.
                                            </p>
                                        </motion.div>
                                    ) : (
                                        <button
                                    ...
                                    <div className="mt-4 grid grid-cols-2 gap-3">
                                        <div className="glass-card p-3 text-center">
                                            <p className="text-lg font-bold text-white">
                                                {campaignResult.content?.distribution?.bestTime ||
                                                    `${Math.floor(Math.random() * 4) + 5}:00 PM IST`}
                                            </p>
                            ...
                            <div className="glass-card p-6">
                                <div className="flex flex-wrap gap-1 mb-6 bg-white/5 rounded-xl p-1">
                                    {[
                                        ...
                                        { key: 'captions', label: '� Captions', condition: true }, // Always show captions
                                        {
                                            key: 'whatsapp',
                                            label: '💬 WhatsApp',
                                            condition: selectedPlatforms.includes('whatsapp'),
                                        },
                                        {
                                            key: 'storyboard',
                                            label: '🎬 Reel',
                                ...
                                {activeTab === 'captions' && (
                                    <div className="space-y-4">
                                        {/* ─── Translate All button + Comprehend ─────────── */}
                                        <div
                                            style={{
                                                display: 'flex',
                                                gap: '0.5rem',
                                        ...
                                        >
                                            <motion.button
                                                ...
                                                onClick={async () => {
                                                    const caption =
                                                        campaignResult?.content?.captions
                                                            ?.instagram ||
                                                        campaignResult?.content?.captions
                                                            ?.summary ||
                                                        Object.values(
                                                    ...
                                                    try {
                                                        ...
                                                        toast.success(
                                                            `🌐 Translated to ${result.languageCount} languages!`
                                                        );
                                                    } catch (e: any) {
                                                        toast.error(
                                                            'Translate failed: ' +
                                                ...
                                                style={{
                                                    ...
                                                    padding: '0.5rem 1rem',
                                                    color: translating ? '#475569' : '#22d3ee',
                                                    fontWeight: 700,
                                                    fontSize: '0.82rem',
                                            ...
                                            >
                                                {translating ? (
                                                    ...
                                                    '🌐 Translate All (Amazon Translate)'
                                                )}
                                            </motion.button>
                                            ...
                                            {comprehendResult && (
                                                ...
                                                >
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            {comprehendResult.sentiment.label ===
                                                            'POSITIVE'
                                                                ? '😊'
                                                                : comprehendResult.sentiment
                                                                        .label === 'NEGATIVE'
                                                    ...
                                                    {comprehendResult.keyPhrases.length > 0 && (
                                                        <div>
                                                            <div
                                                                style={{
                                                                    color: '#94a3b8',
                                        ...
                                        {/* Original captions grid */}
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {Object.entries(campaignResult.content.captions).map(
                                                ([platform, caption]) => (
                                                    <div key={platform} className="glass-card p-4">
                                                        <div className="flex items-center gap-2 mb-2">
                                                            <span className="capitalize font-semibold text-sm text-white">
                                                                {platform}
                                                            </span>
                                                        </div>
                                                        <p className="text-slate-300 text-sm whitespace-pre-line line-clamp-6">
                                                            {caption}
                                                        </p>
                                                        <button
                                                            onClick={() => {
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="glass-card p-4">
                                            ...
                                            <p className="text-slate-300 text-sm">
                                                {campaignResult.content.seo.title}
                                            </p>
                                            <h4 className="font-bold text-white mb-2 text-sm mt-4">
                                                Meta Description
                                            </h4>
                                            <p className="text-slate-300 text-sm">
                                                {campaignResult.content.seo.metaDescription}
                                            </p>
                                        </div>
                                        <div className="glass-card p-4">
                                            <h4 className="font-bold text-white mb-3 text-sm">
                                                Keywords (
                                                {campaignResult.content.seo.keywords.length})
                                            </h4>
                                            <div className="flex flex-wrap gap-2">
                                                {campaignResult.content.seo.keywords.map(kw => (
                                                    <span
                                                        key={kw}
                                                        className="px-2 py-1 text-xs rounded-lg text-cyan-400 bg-cyan-400/10 border border-cyan-400/20"
                                                    >
                                                        {kw}
                                                    </span>
                                                ))}
                                        ...
                                        <div className="glass-card p-5 max-w-sm flex flex-col gap-3">
                                            ...
                                            <div className="flex flex-col gap-2 pt-1">
                                                ...
                                                <motion.button
                                                    ...
                                                    onClick={async () => {
                                                        const statusText =
                                                            campaignResult.content?.whatsapp
                                                                ?.statusText ||
                                                        ...
                                                        if (navigator.share) {
                                                            ...
                                                            toast.success(
                                                                'Status text copied! 📋 Opening WhatsApp Web...'
                                                            );
                                                            setTimeout(
                                                                () =>
                                                                    window.open(
                                                ...
                                                <p className="text-[10px] text-slate-500 text-center font-mono">
                                                    On mobile: tap "Post to Status" → share
                                                    directly.
                                                    <br />
                                                    On desktop: text is copied → paste in WhatsApp
                                                    Web Status.
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        const reelFrames = [
                                            {
                                                time: '0:00',
                                                emoji: '🎬',
                                                visual: `Opening shot focusing on: ${inp}`,
                                                narration: `${inp.split('.')[0] || cap?.instagram?.split('\n')[0]}`,
                                                transition: 'Fade in',
                                            },
                                            {
                                                time: '0:03',
                                                emoji: '✨',
                                                visual: `Dynamic transition highlighting key features from: ${inp}`,
                                                narration: `Experience the best of ${biz}. ${inp.substring(0, 50)}...`,
                                                transition: 'Slide left',
                                            },
                                            {
                                                time: '0:07',
                                                emoji: '🙌',
                                                visual: `Happy customer enjoying ${biz} in ${reg}`,
                                                narration: `${cap?.whatsapp?.split('\n')[2] || `Trusted across ${reg}`}`,
                                                transition: 'Zoom in',
                                            },
                                            {
                                                time: '0:11',
                                                emoji: '🎁',
                                                visual: `Promo offer banner displaying details from prompt`,
                                                narration: `Special offer! Check description for details.`,
                                                transition: 'Flash',
                                            },
                                            {
                                                time: '0:14',
                                                emoji: '📱',
                                                visual: `Contact information and social handles`,
                                                narration: `Follow us for more! #${biz.replace(/\s+/g, '')}`,
                                                transition: 'Fade out',
                                            },
                                        ];
                                        return (
                                ...
                                {activeTab === 'remixer' && campaignResult?.content?.captions && (
                                    <div className="space-y-4">
                                        {Object.entries(campaignResult.content.captions)
                                            .slice(0, 2)
                        ...
                        >
                            <div className="flex items-center gap-4 text-sm font-medium">
                                <div className="flex -space-x-2">
                                    {['🤖', '⚡', '✨'].map((e, i) => (
                                        <div
                                            key={i}
                                            className="w-8 h-8 rounded-full bg-slate-800 border-2 border-slate-900 flex items-center justify-center text-xs relative z-10"
                                            style={{ zIndex: 3 - i }}
                                        >
                                            {e}
                                        </div>
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="px-5 py-3 rounded-xl font-bold transition-all text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 border border-white/5 text-sm flex items-center gap-2"
                                >
                                    🎙️ Auto Fill Demo
                                </button>
                                <button
                                    onClick={handleSubmit}
                                    disabled={!textInput && inputMode === 'text'}
