...
export default function NewCampaign() {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        const pipelineToastId = toast.loading('🤖 AI Pipeline running… Research → Creative → Quality → Distribution', {
    ...
    const handlePublishAll = () => {
        ...
        toast.success(`🚀 Sharing to ${selectedPlatforms.length} platforms!`, {
            description: selectedPlatforms.map(p => p.charAt(0).toUpperCase() + p.slice(1)).join(' · '),
        });
    ...
    const handleDownload = () => {
        ...
        toast.success('Campaign exported! 📥', { description: 'Saved as bharatmedia-campaign.txt' });
    };
                        ...
                        <motion.div key="step3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {campaignId && (
                                        <button
                                            onClick={() => navigate(`/persona-review/${campaignId}`)}
                                            style={{
                                                background: 'linear-gradient(135deg, rgba(99,102,241,0.25), rgba(139,92,246,0.2))',
                                                border: '1px solid rgba(99,102,241,0.4)',
                                                color: '#a5b4fc', borderRadius: '0.5rem',
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.5rem', alignItems: 'center' }}>
                                            <motion.button
                                                ...
                                                onClick={async () => {
                                                    const caption = campaignResult?.content?.captions?.instagram
                                                        || campaignResult?.content?.captions?.summary
                                                        || Object.values(campaignResult?.content?.captions || {})[0]
                                                        || textInput || '';
                                                    if (!caption) return;
                                                    setTranslating(true);
                                                    try {
                                                        const result = await translateCaptions(caption, language);
                                                        setTranslations(result.translations);
                                                        toast.success(`🌐 Translated to ${result.languageCount} languages!`);
                                                    } catch (e: any) {
                                                        toast.error('Translate failed: ' + (e?.response?.data?.message || e.message));
                                                    } finally { setTranslating(false); }
                                                }}
                                                disabled={translating}
                                                style={{
                                                    background: translating ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg,rgba(34,211,238,0.2),rgba(99,102,241,0.2))',
                                                    border: '1px solid rgba(34,211,238,0.4)', borderRadius: '0.65rem',
                                                    padding: '0.5rem 1rem', color: translating ? '#475569' : '#22d3ee',
                                                    fontWeight: 700, fontSize: '0.82rem', cursor: translating ? 'not-allowed' : 'pointer',
                                                    display: 'flex', alignItems: 'center', gap: '0.4rem',
                                                }}
                                            >
                                                {translating
                                                    ? <><motion.span animate={{ rotate: 360 }} transition={{ duration: 0.8, repeat: Infinity, ease: 'linear' }} style={{ display: 'inline-block' }}>⟳</motion.span> Translating…</>
                                            ...
                                            <motion.button
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    setAnalyzing(true);
                                                    try {
                                                        const result = await analyzeCopy(caption, language);
                                                        setComprehendResult(result);
                                                        toast.success('🧠 Comprehend analysis complete!');
                                                    } catch (e: any) {
                                                        toast.error('Comprehend failed: ' + (e?.response?.data?.message || e.message));
                                                    } finally { setAnalyzing(false); }
                                                }}
                                                disabled={analyzing}
                                                style={{
                                                    background: analyzing ? 'rgba(255,255,255,0.05)' : 'rgba(168,85,247,0.15)',
                                                    border: '1px solid rgba(168,85,247,0.4)', borderRadius: '0.65rem',
                                                    padding: '0.5rem 1rem', color: analyzing ? '#475569' : '#a855f7',
                                                    fontWeight: 700, fontSize: '0.82rem', cursor: analyzing ? 'not-allowed' : 'pointer',
                                                    display: 'flex', alignItems: 'center', gap: '0.4rem',
                                                }}
                                            >
                                                {analyzing ? '🧠 Analyzing…' : '🧠 Comprehend Sentiment'}
                                            </motion.button>
                                        </div>
                                            ...
                                            {comprehendResult && (
                                                <motion.div
                                                    ...
                                                    style={{
                                                        ...
                                                        borderRadius: '0.875rem', padding: '1rem', marginBottom: '0.5rem',
                                                    }}
                                                >
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                                                        <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#FF9900', background: 'rgba(255,153,0,0.12)', border: '1px solid rgba(255,153,0,0.25)', borderRadius: 999, padding: '0.1rem 0.5rem' }}>☁️ Amazon Comprehend</span>
                                                        <span style={{
                                                            fontWeight: 800, fontSize: '0.95rem',
                                                            color: comprehendResult.sentiment.label === 'POSITIVE' ? '#4ade80'
                                                                : comprehendResult.sentiment.label === 'NEGATIVE' ? '#f87171'
                                                                : '#fbbf24',
                                                        }}>
                                                            {comprehendResult.sentiment.label === 'POSITIVE' ? '😊' : comprehendResult.sentiment.label === 'NEGATIVE' ? '😞' : '😐'} {comprehendResult.sentiment.label}
                                                        </span>
                                                    </div>
                                                    {comprehendResult.keyPhrases.length > 0 && (
                                                        <div>
                                                            <div style={{ color: '#94a3b8', fontSize: '0.72rem', fontWeight: 700, marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Key Phrases</div>
                                                            <div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap' }}>
                                                                {comprehendResult.keyPhrases.slice(0, 6).map(kp => (
                                                                    <span key={kp.text} style={{
                                                                    ...
                                                                    }}>{kp.text} <span style={{ color: '#64748b' }}>{kp.score}%</span></span>
                                                                ))}
                                                            </div>
                                                        </div>
                                                    )}
                                            ...
                                            {translations && (
                                                ...
                                                >
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                                                        <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#FF9900', background: 'rgba(255,153,0,0.12)', border: '1px solid rgba(255,153,0,0.25)', borderRadius: 999, padding: '0.1rem 0.5rem' }}>☁️ Amazon Translate</span>
                                                        <span style={{ color: '#22d3ee', fontWeight: 700, fontSize: '0.82rem' }}>🌐 {Object.keys(translations).length} Languages</span>
                                                        <button onClick={() => setTranslations(null)} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: '0.8rem' }}>✕</button>
                                                    </div>
                                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem' }}>
                                                        {Object.entries(translations).map(([lang, txt]) => (
                                                            <div key={lang} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '0.5rem', padding: '0.6rem' }}>
                                                                <div style={{ color: '#22d3ee', fontSize: '0.68rem', fontWeight: 700, marginBottom: '0.25rem', textTransform: 'uppercase' }}>{lang}</div>
                                                                <div style={{ color: '#cbd5e1', fontSize: '0.78rem', lineHeight: 1.4 }}>{String(txt).slice(0, 120)}{String(txt).length > 120 ? '…' : ''}</div>
                                                                <button onClick={() => { navigator.clipboard.writeText(String(txt)); toast.success(`Copied ${lang} translation!`); }}
                                                                    style={{ marginTop: '0.3rem', background: 'none', border: 'none', color: '#22d3ee', cursor: 'pointer', fontSize: '0.68rem', padding: 0 }}>📋 Copy</button>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </motion.div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {Object.entries(campaignResult.content.captions).map(([platform, caption]) => (
                                                <div key={platform} className="glass-card p-4">
                                                    ...
                                                    <button onClick={() => { navigator.clipboard.writeText(String(caption)); toast.success('Caption copied! 📋'); }}
                                                ...
                                                <motion.button
                                                    ...
                                                    onClick={async () => {
                                                        ...
                                                        if (navigator.share) {
                                                            ...
                                                            toast.success('Status text copied! 📋 Opening WhatsApp Web...');
