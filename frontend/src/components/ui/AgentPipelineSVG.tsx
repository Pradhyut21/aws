...
export default function AgentPipelineSVG({ stages, isComplete, isAborted, contentHash, onAbort }: Props) {
        ...
        <div style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
            ...
            >
                ...
                {STAGE_CONFIG.map((_, i) => {
                    ...
                    return (
                        <line key={i}
                            x1={x1} y1={NODE_CY} x2={x2} y2={NODE_CY}
                ...
                {STAGE_CONFIG.map((cfg, i) => {
                    ...
                    return (
                        <g key={i}>
                            ...
                            {isRunning && (
                                <>
                                    <circle cx={cx} cy={NODE_CY} r={NODE_R + 8} fill="none"
                                        stroke={cfg.color} strokeWidth={1.5} opacity={0.4}
                                        style={{ animation: 'pulse-ring 1.4s ease-out infinite' }} />
                                    <circle cx={cx} cy={NODE_CY} r={NODE_R + 14} fill="none"
                                        stroke={cfg.color} strokeWidth={1} opacity={0.2}
                                        style={{ animation: 'pulse-ring 1.4s ease-out infinite 0.3s' }} />
                                </>
                            )}

                            {/* Node circle */}
                            <circle cx={cx} cy={NODE_CY} r={NODE_R}
                                fill={fill}
                            ...
                            {/* Icon */}
                            <text x={cx} y={NODE_CY + 1} textAnchor="middle" dominantBaseline="middle"
                                fontSize={18} style={{ userSelect: 'none' }}>
                                {cfg.icon}
                            </text>

                            {/* Label below */}
                            <text x={cx} y={NODE_CY + NODE_R + 14} textAnchor="middle"
                                fontSize={10} fill={status === 'waiting' ? '#475569' : '#94a3b8'}
                            ...
                            {/* Status badge */}
                            {status !== 'waiting' && (
                                <text x={cx} y={NODE_CY + NODE_R + 26} textAnchor="middle"
                                    fontSize={9}
                                    fill={status === 'done' ? '#4ade80' : status === 'error' ? '#f87171' : cfg.color}
                                    fontFamily="Inter, system-ui, sans-serif">
                            ...
                            {stage?.detail && status === 'running' && (
                                <foreignObject x={cx - 55} y={NODE_CY - NODE_R - 30} width={110} height={28}>
                                    <div style={{
                                        color: '#94a3b8', fontSize: '0.6rem', textAlign: 'center',
            ...
            {/* Aborted state */}
            {isAborted && (
                <div style={{ textAlign: 'center', marginTop: '0.5rem', color: '#f87171', fontSize: '0.78rem' }}>
                    ⬛ Campaign aborted
                </div>
            )}
            ...
            {isComplete && contentHash && (
                <div style={{
                    ...
                    <div style={{ color: '#4ade80', fontSize: '0.7rem', fontWeight: 600, marginBottom: '0.1rem' }}>
                        🔒 Content Integrity Hash
                    </div>
                    <div style={{ color: '#64748b', fontFamily: 'monospace', fontSize: '0.65rem', wordBreak: 'break-all' }}>
                        SHA-256: {contentHash}
                    </div>
                </div>
            )}
