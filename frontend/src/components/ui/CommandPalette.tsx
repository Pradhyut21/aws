...
function loadCampaigns(): any[] {
    try {
        return JSON.parse(localStorage.getItem('bm_campaigns') || '[]');
    } catch {
        return [];
    }
}
...
function quickBharatScore(text: string): number {
    ...
    indianSlang.forEach(s => {
        if (lower.includes(s)) score += 6;
    });
    festivals.forEach(f => {
        if (lower.includes(f)) score += 8;
    });
    urgency.forEach(u => {
        if (lower.includes(u)) score += 4;
    });
    ...
    const staticItems: PaletteItem[] = useMemo(
        () => [
            ...
            },
            {
                ...
                icon: '🔍',
                label: 'Agent Trace',
                sublabel: 'Debug AI pipeline steps',
                category: 'Navigate',
                action: () => {
                    navigate('/trace');
                    close();
                },
                keywords: ['debug', 'agents', 'pipeline'],
            },
            {
                id: 'nav-brain',
                icon: '🧠',
                label: 'BharatBrain',
                sublabel: 'AI knowledge graph',
                category: 'Navigate',
                action: () => {
                    navigate('/brain');
                    close();
            ...
            },
            {
                ...
                icon: '📡',
                label: 'Market Pulse',
                sublabel: 'Live trend signals',
                category: 'Navigate',
                action: () => {
                    navigate('/market-pulse');
                    close();
                },
                keywords: ['trends', 'signals'],
            },
            {
                id: 'nav-exp',
                icon: '⚗️',
                label: 'Experiment Lab',
                sublabel: 'A/B test campaigns',
                category: 'Navigate',
                action: () => {
                    navigate('/experiments');
                    close();
                },
                keywords: ['ab test', 'variants'],
            },
            {
                id: 'nav-templates',
                icon: '📁',
            ...
            // Quick audit trigger
            {
                id: 'audit',
                icon: '⚡',
                label: 'Instant BharatScore Audit',
                sublabel: 'Paste ad copy → get cultural resonance',
                category: 'Tools',
                action: () => setShowAudit(true),
                keywords: ['score', 'audit', 'test', 'check'],
            },
            // GitHub Ideas — Cultural & AI Features
            {
                id: 'nav-focus',
                icon: '💬',
                label: 'Bharat Focus Group',
                sublabel: 'WhatsApp persona debate simulator',
                category: 'Cultural AI',
                action: () => {
                    navigate('/focus-group');
                    close();
                },
                keywords: ['personas', 'debate', 'whatsapp', 'focus'],
            },
            {
                id: 'nav-map',
                icon: '🗺️',
                label: 'Bharat Dialect Atlas',
                sublabel: 'Interactive regional cultural map',
                category: 'Cultural AI',
                action: () => {
                    navigate('/dialect-atlas');
                    close();
                },
                keywords: ['map', 'states', 'regions', 'india'],
            },
            {
                id: 'nav-diff',
                icon: '✨',
                label: 'Cultural Diff Inspector',
                sublabel: 'Slang & localized copy side-by-side',
                category: 'Cultural AI',
                action: () => {
                    navigate('/cultural-diff');
                    close();
                },
                keywords: ['diff', 'slang', 'localize', 'hinglish'],
            },
            {
                id: 'nav-sensitivity',
                icon: '🛡️',
                label: 'Sensitivity Matrix',
                sublabel: '6-point cultural safety audit',
                category: 'Cultural AI',
                action: () => {
                    navigate('/sensitivity');
                    close();
                },
                keywords: ['taboo', 'safety', 'guard', 'audit', 'asci'],
            },
            {
                id: 'nav-audio',
                icon: '🎙️',
                label: 'Audio Wave Studio',
                sublabel: 'Regional voice & waveform generator',
                category: 'Creator Studio',
                action: () => {
                    navigate('/audio-studio');
                    close();
                },
                keywords: ['voice', 'audio', 'rj', 'waveform', 'sound'],
            },
            {
                id: 'nav-poster',
                icon: '🎨',
                label: 'Festival Poster Studio',
                sublabel: '1-click WhatsApp & Instagram poster',
                category: 'Creator Studio',
                action: () => {
                    navigate('/poster-studio');
                    close();
                },
                keywords: ['poster', 'diwali', 'festival', 'whatsapp', 'instagram'],
            },
            {
                id: 'nav-trans',
                icon: '⌨️',
                label: 'Transliteration Engine',
                sublabel: 'Hinglish / Tanglish 3-way converter',
                category: 'Creator Studio',
                action: () => {
                    navigate('/transliteration');
                    close();
                },
                keywords: ['transliterate', 'hinglish', 'tanglish', 'script', 'devanagari'],
            },
            {
                id: 'nav-ideas',
                icon: '🇮🇳',
                label: 'Ideas Hub — All 8 Innovations',
                sublabel: 'Browse all 8 GitHub-inspired Bharat features',
                category: 'Navigate',
                action: () => {
                    navigate('/ideas');
                    close();
                },
                keywords: ['ideas', 'hub', 'innovations', 'github', 'features', 'showcase'],
            },
        ],
        [navigate, close]
    );
    ...
    const recentItems: PaletteItem[] = useMemo(
        () =>
            campaigns.slice(0, 5).map((c: any, i: number) => ({
                ...
                icon: '📋',
                label:
                    c.input?.substring(0, 40) + (c.input?.length > 40 ? '…' : '') ||
                    `Campaign ${i + 1}`,
                ...
                {open && (
                    <motion.div
                        ...
                        style={{
                            ...
                            boxShadow:
                                '0 0 0 1px rgba(255,107,53,0.1), 0 32px 80px rgba(0,0,0,0.7), 0 0 60px rgba(255,107,53,0.06)',
                            overflow: 'hidden',
                                ...
                                >
                                    <div
                                        style={{
                                            ...
                                            fontSize: '1.1rem',
                                            fontWeight: 800,
                                            color: scoreLabel(auditScore).color,
                                    ...
                                    <div>
                                        ...
                                        >
                                            {auditScore >= 70
                                                ? '✅ Strong regional resonance — ready to publish'
                                                : auditScore >= 50
                                                  ? '⚡ Moderate — add local slang or festival hook to boost score'
