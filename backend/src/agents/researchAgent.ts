...
const researchTool = tool<{ campaign: Campaign; lessons: string[] }, ResearchOutput>({
    ...
    description:
        'Research market trends, audience demographics, and competitor insights for an Indian SMB campaign',
...
export async function runResearchAgent(campaign: Campaign): Promise<ResearchOutput> {
    ...
    try {
        ...
        lessons = storedLessons
            .filter(
                l =>
                    l.businessType === campaign.businessType ||
                    l.region.some(r => campaign.region.includes(r))
            )
            .map(l => l.lesson);
    } catch {
    ...
    try {
        const output = (await agent.invoke(
            JSON.stringify({ campaign, lessons })
        )) as ResearchOutput;
        ...
        return {
            trendingFormats: [
                'Short-form video with local music',
                'Carousel showing process/behind-scenes',
            ...
            hashtags: [
                ...
                '#VocalForLocal',
                '#MadeInIndia',
                `#${(campaign.region[0] || 'India').replace(/\s+/g, '')}`,
                '#IndianSMB',
                '#LocalBusiness',
            ],
            culturalContext: `Authentic local craftsmanship from ${campaign.region[0] || 'India'} — emphasise heritage and quality over price`,
            competitorInsights:
                'Competitors focus on discounts; opportunity to differentiate on quality, story, and cultural authenticity',
            evidence: [
                'Fallback: Bedrock call unavailable — using regional knowledge base defaults',
            ],
