...
export interface BharatBrainDoc {
    ...
    userId: string;
    name: string;
    type: 'brand_guidelines' | 'product_catalog' | 'persona' | 'previous_campaign' | 'reviews' | 'competitor' | 'other';
...
export async function getUserExperiments(userId: string): Promise<Experiment[]> {
    ...
    return (result.Items || []).map(({ PK, SK, GSI1PK, GSI1SK, _type, ...rest }) => rest as Experiment);
}

export async function updateExperiment(expId: string, updates: Partial<Experiment>): Promise<Experiment | null> {
...
export function estimateCostUsd(modelId: string, inputTokens: number, outputTokens: number): number {
    ...
    if (!costs) return 0;
    return parseFloat(((costs.input * inputTokens + costs.output * outputTokens) / 1000).toFixed(6));
}
